...
// Parse AW SKU format: PREFIX:MFR-PROD-MODEL#-HSO-CARRIER-STORAGE-COLOR-GRADE
// Grade is always last segment. INTAKE SKUs end with -INTAKE.
function parseSKU(sku: string) {
  if (!sku) return null

  try {
    // Split prefix from rest: "PA:APPL-IPH-15PM-HSO-UNL-256-BLK-CA"
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    // Transform items with parsed SKU data
