...
export default function Admin() {
  ...
  const [page,setPage] = useState<'applications'|'messages'|'sync'|'quotes'|'customers'|'employees'|'families'|'pricing'|'skupricing'>('applications')
  ...
  // SKU Pricing Overrides
  const [skuOverrides,setSkuOverrides]             = useState<any[]>([])
  const [skuOverridesLoading,setSkuOverridesLoading] = useState(false)
  const [skuSearch,setSkuSearch]                   = useState("")
  const [skuSearchResults,setSkuSearchResults]     = useState<any[]>([])
  const [skuSearching,setSkuSearching]             = useState(false)
  const [skuPriceInput,setSkuPriceInput]           = useState<Record<string,string>>({})
  const [skuNoteInput,setSkuNoteInput]             = useState<Record<string,string>>({})
  ...
  const authHeaders = (tok?: string|null): Record<string,string> => ({
    ...
    ...((tok !== undefined ? tok : adminToken) ? { 'Authorization': 'Bearer ' + (tok !== undefined ? tok : adminToken) } : {}),
  })
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if(r.ok){
        ...
        setAdminToken(token)
        setAuthed(true)
        sessionStorage.setItem('aw-admin-token', token)
      } else {
  ...
  useEffect(()=>{
    const stored = sessionStorage.getItem('aw-admin-token')
    if(stored){ setAdminToken(stored); setAuthed(true) }
  },[])

  // ── Apps / Msgs ──
  const fetchApps = async (tok?: string|null) => { try{const r=await fetch('/api/admin-applications',{headers:authHeaders(tok)});if(r.ok)setApps(await r.json())}catch{} }
  const fetchMsgs = async (tok?: string|null) => { try{const r=await fetch('/api/admin-messages',{headers:authHeaders(tok)});if(r.ok)setMsgs(await r.json())}catch{} }
  useEffect(()=>{if(!authed||!adminToken)return;setLoading(true);Promise.all([fetchApps(adminToken),fetchMsgs(adminToken)]).finally(()=>setLoading(false))},[authed,adminToken])

  const updateAppStatus = async (id:number,status:string) => {
    try{await fetch('/api/admin-applications',{method:'PATCH',headers:authHeaders(),body:JSON.stringify({id,status})});setApps(p=>p.map(a=>a.id===id?{...a,status}:a));setDetail(null)}catch{}
  }
  const markRead = async (msg:Message) => {
    setMsgDetail(msg)
    if(!msg.read){
      try{await fetch('/api/admin-messages',{method:'PATCH',headers:authHeaders(),body:JSON.stringify({id:msg.id,read:true})});setMsgs(p=>p.map(m=>m.id===msg.id?{...m,read:true}:m))}catch{}
    }
  }
  const sendReply = async () => {
    ...
    try{
      const r=await fetch('/api/send-email',{method:'POST',headers:authHeaders(),body:JSON.stringify({to:msgDetail.email,subject:'Re: '+msgDetail.subject,message:replyText,replyToMsgId:msgDetail.id})})
  ...
  const requestDocs = async () => {
    ...
    setDocsSending(true)
    try{
      const r=await fetch('/api/request-docs',{method:'POST',headers:authHeaders(),body:JSON.stringify({applicationId:docsModal.id,email:docsModal.email,companyName:docsModal.company_name,firstName:docsModal.first_name,documents:selectedDocs})})
  ...
  const approveApp = async (app:Application) => {
    setApproving(app.id)
    try{
      const r=await fetch('/api/approve-application',{method:'POST',headers:authHeaders(),body:JSON.stringify({applicationId:app.id})})
  ...
  // ── Sync ──
  const testSCAuth = async () => { setSyncLoading(true);try{const r=await fetch('/api/sc/auth',{headers:authHeaders()});setSyncStatus(await r.json())}catch(e){setSyncStatus({error:String(e)})}finally{setSyncLoading(false)} }
  const fetchSCCatalog = async (kw?:string) => { setSyncSearching(true);try{const r=await fetch(kw?'/api/sc/catalog?size=20&active=false&keyword='+encodeURIComponent(kw):'/api/sc/catalog?size=20&active=false',{headers:authHeaders()});setCatalogData(await r.json())}catch(e){setCatalogData({error:String(e)})}finally{setSyncSearching(false)} }
  const fetchSCInventory = async (kw?:string) => { setSyncSearching(true);try{const r=await fetch(kw?'/api/sc/inventory?size=20&inStockOnly=true&keyword='+encodeURIComponent(kw):'/api/sc/inventory?size=20&inStockOnly=true',{headers:authHeaders()});setInventoryData(await r.json())}catch(e){setInventoryData({error:String(e)})}finally{setSyncSearching(false)} }
  useEffect(()=>{if(page==='sync'&&!syncStatus)testSCAuth()},[page])

  // ── Quotes ──
  const fetchQuotes = useCallback(async () => {
    setQuotesLoading(true)
    try{const r=await fetch('/api/submit-quote',{headers:authHeaders()});const d=await r.json();setQuotes(d.quotes||[])}
  ...
  const updateQuote = async (id:number,status:string,reason?:string) => {
    ...
    try{
      const r=await fetch('/api/submit-quote',{method:'PATCH',headers:authHeaders(),body:JSON.stringify({id,status,declineReason:reason||null})})
  ...
  const fetchCustomers = useCallback(async () => {
    setCustLoading(true)
    try{const r=await fetch('/api/admin-users',{headers:authHeaders()});if(r.ok)setCustomers(await r.json())}
  ...
  const toggleCustomer = async (id:number,active:boolean) => {
    setCustToggling(id)
    try{
      const r=await fetch('/api/admin-toggle-user',{method:'PATCH',headers:authHeaders(),body:JSON.stringify({id,active})})
  ...
  const fetchFamilies = useCallback(async () => {
    ...
    try{const r=await fetch('/api/sc/admin-families',{headers:authHeaders()});if(r.ok)setFamiliesData(await r.json())}
  ...
  const toggleFamOpen = async (modelCode:string) => {
    ...
    if(!famSkus[modelCode]){
      ...
      try{
        const r=await fetch('/api/sc/admin-families?family='+encodeURIComponent(modelCode),{headers:authHeaders()})
  ...
  const toggleSkuVisible = async (sku:string,hidden:boolean,modelCode:string) => {
    try{
      const r=await fetch('/api/sc/admin-families',{method:'PATCH',headers:authHeaders(),body:JSON.stringify({type:'sku',sku,hidden})})
      if(r.ok){
        setFamSkus(p=>{
  ...
  const bulkHideFamily = async (modelCode:string,name:string) => {
    try{
      const r=await fetch('/api/sc/admin-families',{method:'PATCH',headers:authHeaders(),body:JSON.stringify({type:'bulk-hide',modelCode})})
      if(r.ok){
        setFamSkus(p=>{
          const d=p[modelCode];if(!d)return p
  ...
  const renameFamily = async (id:number,currentName:string) => {
    ...
    try{
      const r=await fetch('/api/sc/admin-families',{method:'PATCH',headers:authHeaders(),body:JSON.stringify({type:'rename',id,name:n})})
  ...
  const createFamily = async () => {
    ...
    try {
      ...
      if (r.ok) {
        showToast('Family created'); setCreateFamModal(false)
        setCreateFamForm({ modelCode: '', name: '', brand: 'Apple', category: 'Phones', visible: true })
        fetchFamilies()
      } else { const d = await r.json(); showToast(d.error || 'Create failed', true) }
  ...
  const fetchPricing = useCallback(async () => {
    ...
    try{const r=await fetch('/api/sc/pricing',{headers:authHeaders()});if(r.ok){const d=await r.json();setPricingData(d);setPricingGrid(d.grid||{})}}
  ...
  const savePricing = async () => {
    ...
    setPricingSaving(true); let errors=0
    for(const cat of(pricingData.categories||[])){
      for(const grade of(pricingData.grades||[])){
        const val=pricingGrid[cat]?.[grade.code]; if(val===undefined)continue
        try{ const r=await fetch('/api/sc/pricing',{method:'POST',headers:authHeaders(),body:JSON.stringify({category:cat,grade_code:grade.code,multiplier:val})}); if(!r.ok)errors++ }catch{errors++}
      }
    }
  ...
  // ── SKU Overrides ──
  const fetchSkuOverrides = useCallback(async () => {
    setSkuOverridesLoading(true)
    try{const r=await fetch('/api/sc/sku-overrides',{headers:authHeaders()});if(r.ok)setSkuOverrides(await r.json())}
    catch{showToast('Failed to load overrides',true)}
    finally{setSkuOverridesLoading(false)}
  },[adminToken])
  useEffect(()=>{if(authed&&page==='skupricing')fetchSkuOverrides()},[authed,page])

  const searchSkus = async () => {
    if(!skuSearch.trim())return
    setSkuSearching(true)
    try{const r=await fetch('/api/sc/pricing?search='+encodeURIComponent(skuSearch),{headers:authHeaders()});if(r.ok){const d=await r.json();setSkuSearchResults(d.results||[])}}
    catch{showToast('Search failed',true)}
    finally{setSkuSearching(false)}
  }

  const saveSkuOverride = async (sku:string) => {
    const price = parseFloat(skuPriceInput[sku])
    if(isNaN(price)||price<=0){showToast('Enter a valid price',true);return}
    setSkuSaving(sku)
    try{
      const r=await fetch('/api/sc/sku-overrides',{method:'POST',headers:authHeaders(),body:JSON.stringify({sku,price,note:skuNoteInput[sku]||null})})
      if(r.ok){showToast('Override saved — live immediately');fetchSkuOverrides()}
      else showToast('Save failed',true)
    }catch{showToast('Save failed',true)}finally{setSkuSaving(null)}
  }

  const deleteSkuOverride = async (sku:string) => {
    setSkuSaving(sku)
    try{
      const r=await fetch('/api/sc/sku-overrides',{method:'DELETE',headers:authHeaders(),body:JSON.stringify({sku})})
      if(r.ok){showToast('Override removed — back to multiplier pricing');setSkuOverrides(p=>p.filter(o=>o.sku!==sku))}
      else showToast('Delete failed',true)
    }catch{showToast('Delete failed',true)}finally{setSkuSaving(null)}
  }
  ...
  const PAGE_TITLES:Record<string,string> = {
    ...
    families:'Product Families',pricing:'Grade Multipliers',skupricing:'SKU Price Overrides',
  }
  ...
  return (
    <div style={{display:'flex',minHeight:'100vh',background:C.bg,fontFamily:"'DM Sans',system-ui,sans-serif",color:C.text,WebkitFontSmoothing:'antialiased'}}>
      <div style={{width:220,background:C.dark,borderRight:'1px solid '+C.border,position:'fixed',top:0,left:0,bottom:0,zIndex:50,display:'flex',flexDirection:'column',overflowY:'auto'}}>
        ...
        {sbItem('skupricing','💲','SKU Overrides')}
                  ...
                  <table style={{width:'100%',borderCollapse:'collapse'}}>
                    ...
                    <tbody>
                      {quotes.map(q=>{const sb=statusBadge(q.status);return(
                        <tr key={q.id}>
                          <td style={td(true)}>{q.ref_number}</td><td style={td(true)}>{q.company_name}</td>
                  ...
                  <table style={{width:'100%',borderCollapse:'collapse'}}>
                    ...
                    <tbody>
                      {filteredCustomers.map(c=>{const tb=typeBadge(c.account_type);return(
                        <tr key={c.id}>
                          <td style={td(true)}>{c.company_name}</td><td style={td()}>{c.first_name} {c.last_name}</td>
              ...
              <table style={{width:'100%',borderCollapse:'collapse'}}>
                ...
                <tbody>
                  {employees.map(e=>{
                    ...
                    return(
                      <tr key={e.id}>
                        <td style={td(true)}>{e.first} {e.last}</td><td style={td()}>{e.email}</td>
                              ...
                              <tbody>
                                {allSkus.map((s:any)=>{const gb=gradeBadge(s.grade);return(
                                  <tr key={s.sku} style={{opacity:s.hidden?0.45:1}}>
                                    ...
                                    <td style={td()}>{s.storage||'—'}</td><td style={td()}>{s.carrier||'—'}</td>
                  ...
                  <tbody>
                    {familiesData.unmapped.map((u:any,i:number)=>(
                      <tr key={i}>
                        ...
                        <td style={td()}>{u.model||'—'}</td><td style={td()}>{u.brand||'—'}</td>
                ...
                return(
                  <div key={cat} style={{...card(),marginBottom:16}}>
                    ...
                    <div style={{overflowX:'auto',padding:'8px 0'}}>
                      <table style={{width:'100%',borderCollapse:'collapse'}}>
                        <thead><tr>{grades.map((g:any)=>{const gb=gradeBadge(g.code);return(<th key={g.code} style={{...th(),textAlign:'center',paddingBottom:12}}><div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:4}}><Badge color={gb.color} bg={gb.bg} label={g.code}/><span style={{fontSize:10,color:C.dim,fontWeight:400}}>{g.label}</span></div></th>)})}</tr></thead>
                        <tbody><tr>{grades.map((g:any)=>(<td key={g.code} style={{...td(),textAlign:'center',padding:'8px 12px'}}><input type="number" step="0.01" min="0.5" max="5" value={pricingGrid[cat]?.[g.code]??''} onChange={e=>{const v=parseFloat(e.target.value);if(isNaN(v))return;setPricingGrid(p=>({...p,[cat]:{...(p[cat]||{}),[g.code]:v}}));setPricingDirty(true)}} style={{width:68,textAlign:'center',padding:'7px 4px',background:C.dark,border:'1px solid '+(pricingDirty&&pricingGrid[cat]?.[g.code]!==pricingData.grid?.[cat]?.[g.code]?C.orange:C.border),borderRadius:6,fontSize:13,color:C.text,fontFamily:'inherit',outline:'none'}}/>{pricingGrid[cat]?.[g.code]&&<div style={{fontSize:10,color:C.dim,marginTop:3}}>×{pricingGrid[cat][g.code]}</div>}</td>))}</tr></tbody>
                      </table>
                    </div>
                  </div>
                )
        ...
        <div style={{...modal(),width:440}} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{display:'flex',gap:8,paddingTop:20,borderTop:'1px solid '+C.border}}>
            <button style={btn('primary')} onClick={async ()=>{
              try{
                await fetch('/api/sc/admin-families',{method:'PATCH',headers:authHeaders(),body:JSON.stringify({type:'family',id:photoModal.famId,image_url:photoUrl})})
      ...
      {createFamModal&&<div style={overlay()} onClick={()=>setCreateFamModal(false)}>
        <div style={{...modal(),width:480}} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14}}>
            <div><div style={mlabel()}>Brand</div><select style={inp()} value={createFamForm.brand} onChange={e=>setCreateFamForm(p=>({...p,brand:e.target.value}))}>{['Apple','Samsung','Google','Motorola','Nokia','Other'].map(b=><option key={b}>{b}</option>)}</select></div>
            <div><div style={mlabel()}>Category</div><select style={inp()} value={createFamForm.category} onChange={e=>setCreateFamForm(p=>({...p,category:e.target.value}))}>{['Phones','Tablets','Laptops','Wearables','Accessories'].map(c=><option key={c}>{c}</option>)}</select></div>
          </div>
          ...
          <div style={{display:'flex',gap:8,paddingTop:20,borderTop:'1px solid '+C.border}}>
            <button style={btn('primary')} disabled={createFamSaving||!createFamForm.modelCode||!createFamForm.name} onClick={createFamily}>{createFamSaving?'Creating...':'Create Family'}</button>
