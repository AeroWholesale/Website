...
const css = `
  ...
  .aw-upload { font-family: 'DM Sans', sans-serif; background: #f8fafc; min-height: 100vh; -webkit-font-smoothing: antialiased; }
  ...
  .aw-upload-item-btn button { background: #132347; color: #fff; font-size: 12px; font-weight: 700; padding: 8px 16px; border-radius: 6px; border: none; cursor: pointer; font-family: 'DM Sans', sans-serif; transition: background 0.15s; }
  .aw-upload-item-btn button:hover { background: #1a2f5e; }
  ...
  .aw-upload-progress { height: 4px; background: #e2e8f0; border-radius: 2px; margin-top: 6px; overflow: hidden; }
  .aw-upload-progress-bar { height: 100%; background: #c2410c; border-radius: 2px; transition: width 0.3s; }
...
export default function Upload() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/upload-info?token=${token}`)
      ...
      .then(data => {
        ...
        data.uploaded?.forEach((u: any) => { existing[u.docType] = u.fileName })
  ...
  const handleUpload = async (docType: string, file: File) => {
    ...
    try {
      const res = await fetch(`/api/upload-file?token=${token}&docType=${docType}&fileName=${encodeURIComponent(file.name)}`, {
        method: 'POST',
        body: file,
      })
      if (res.ok) {
    ...
    <>
      ...
      <div className="aw-upload">
        ...
        <div className="aw-upload-main">
          {loading ? (
            <div className="aw-upload-loading">Loading...</div>
          ) : error ? (
            ...
            <div className="aw-upload-card">
              <div className="aw-upload-done">
                ...
                <div className="aw-upload-done-sub">
                  Thank you, {info?.firstName}. We've received all the requested documents for {info?.companyName}.<br /><br />
                  Our team will review everything and you'll hear from us within 1 business day.
                </div>
              </div>
            </div>
          ) : (
            <div className="aw-upload-card">
              <div className="aw-upload-card-header">
                ...
                <div className="aw-upload-sub">
                  Hi {info?.firstName} — please upload the following documents for {info?.companyName} to complete your account setup.
                </div>
              </div>
                      ...
                      <div className="aw-upload-item-info">
                        ...
                        {isDone ? (
                          ...
                          <>
                            ...
                            <div className="aw-upload-progress"><div className="aw-upload-progress-bar" style={{ width: '60%' }} /></div>
                          </>
                        ) : (
                          <div className="aw-upload-item-status">PDF, JPG, or PNG · Max 10MB</div>
                        )}
                      </div>
                      {!isDone && (
                        <div className="aw-upload-item-btn">
                          <button disabled={isUploading}>{isUploading ? '...' : 'Upload'}</button>
                          {!isUploading && (
                            <input
                              type="file"
