...
const css = `
  ...
  .aw-admin-btn-processing { background: #0c1a3a; color: #60a5fa; border-color: #1e3a8a; }
  .aw-admin-btn-processing:hover { background: #1e3a8a; }
  .aw-admin-btn-reset { background: #1e2d4a; color: #94a3b8; border-color: #334155; }
  .aw-admin-btn-reset:hover { background: #334155; }
  ...
  .aw-admin-detail-actions { display: flex; gap: 8px; flex-wrap: wrap; padding-top: 20px; border-top: 1px solid #1e2d4a; }
  .aw-admin-detail-close { margin-left: auto; background: #1e2d4a; color: #94a3b8; border-color: #334155; }

  .aw-admin-reply-textarea { width: 100%; min-height: 100px; padding: 12px; background: #0a0f1a; border: 1px solid #1e2d4a; border-radius: 8px; font-size: 13px; color: #e2e8f0; font-family: 'DM Sans', sans-serif; outline: none; resize: vertical; margin-top: 12px; box-sizing: border-box; }
  ...
  .aw-decline-row { display: flex; gap: 8px; align-items: center; }
  .aw-decline-select { padding: 4px 8px; border-radius: 5px; border: 1px solid #7f1d1d; background: #1c0606; color: #94a3b8; font-size: 11px; font-family: 'DM Sans', sans-serif; cursor: pointer; outline: none; }
  .aw-decline-select:focus { border-color: #ef4444; }
`
...
type QuoteRequest = {
  id: number; ref_number: string; dealer_email: string; dealer_name: string;
  company_name: string; items: QuoteItem[]; notes?: string;
  total_units: number; total_value: string; status: string;
  decline_reason?: string; created_at: string; updated_at: string;
}
...
function statusBadge(status: string) {
  const map: Record<string, string> = {
    ...
    confirmed: 'aw-admin-badge-green',
    processing: 'aw-admin-badge-blue',
    declined: 'aw-admin-badge-red',
  }
...
const DECLINE_REASONS: Record<string, string> = {
  ...
  min_order: 'Minimum order not met',
...
export default function Admin() {
  ...
  const [page, setPage] = useState<'applications' | 'messages' | 'quotes' | 'sync'>('applications')
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (res.ok) { setAuthed(true); sessionStorage.setItem('aw-admin', '1') }
  ...
  const fetchApps = async () => {
    try { const res = await fetch('/api/admin-applications'); if (res.ok) setApps(await res.json()) } catch {}
  }
  const fetchMsgs = async () => {
    try { const res = await fetch('/api/admin-messages'); if (res.ok) setMsgs(await res.json()) } catch {}
  }
  const fetchQuotes = async () => {
    setQuotesLoading(true)
    try { const res = await fetch('/api/submit-quote'); if (res.ok) { const d = await res.json(); setQuotes(d.quotes || []) } }
  ...
  useEffect(() => { if (authed && page === 'quotes') fetchQuotes() }, [page, authed])
  ...
  const updateQuoteStatus = async (id: number, status: string, reason?: string) => {
    try {
      await fetch('/api/submit-quote', {
        method: 'PATCH', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id, status, ...(reason ? { declineReason: reason } : {}) }),
      })
      setQuotes(prev => prev.map(q => q.id === id ? { ...q, status, decline_reason: reason } : q))
      setQuoteDetail(null)
      setDeclineReason('')
      showToast(status === 'confirmed' ? 'Quote confirmed — dealer notified' : status === 'processing' ? 'Marked as processing' : status === 'declined' ? 'Quote declined — dealer notified' : 'Reset to pending')
    } catch { showToast('Failed to update quote', true) }
  }
  ...
  const sendReply = async () => {
    ...
    try {
      const res = await fetch('/api/send-email', {
        ...
        body: JSON.stringify({ to: msgDetail.email, subject: `Re: ${msgDetail.subject}`, message: replyText, replyToMsgId: msgDetail.id }),
      })
      if (res.ok) { showToast('Reply sent'); setMsgDetail(null); setReplyText('') }
      else showToast('Failed to send reply', true)
  ...
  const requestDocs = async () => {
    ...
    try {
      const res = await fetch('/api/request-docs', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ applicationId: docsModal.id, email: docsModal.email, companyName: docsModal.company_name, firstName: docsModal.first_name, documents: selectedDocs }),
      })
      if (res.ok) {
        showToast('Document request sent')
        setApps(prev => prev.map(a => a.id === docsModal.id ? { ...a, status: 'docs_requested' } : a))
        setDocsModal(null); setSelectedDocs([]); setDetail(null)
  ...
  const approveApp = async (app: Application) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setDetail(null)
      } else { const d = await res.json(); showToast(d.error || 'Approval failed', true) }
  ...
  const testSCAuth = async () => {
    setSyncLoading(true)
    try { const res = await fetch('/api/sc/auth'); setSyncStatus(await res.json()) }
    catch (err) { setSyncStatus({ error: String(err) }) }
  ...
  const fetchSCCatalog = async (keyword?: string) => {
    ...
    try {
      ...
      setCatalogData(await (await fetch(url)).json())
    } catch (err) { setCatalogData({ error: String(err) }) }
    finally { setSyncSearching(false) }
  }
  ...
  const fetchSCInventory = async (keyword?: string) => {
    ...
    try {
      ...
      setInventoryData(await (await fetch(url)).json())
    } catch (err) { setInventoryData({ error: String(err) }) }
    finally { setSyncSearching(false) }
  }
      ...
      {!authed ? (
        <div className="aw-admin-login">
          <div className="aw-admin-login-card">
            ...
            <input className="aw-admin-login-input" type="password" placeholder="Password" value={pw}
              onChange={e => setPw(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleLogin()} autoFocus />
        ...
        <div className="aw-admin">
          ...
          <div className="aw-admin-sb">
            ...
            <div className={`aw-admin-sb-item${page === 'quotes' ? ' active' : ''}`} onClick={() => setPage('quotes')}>
              <span>📋</span> Quote Requests
              {pendingQuotes.length > 0 && <span className="aw-admin-sb-badge">{pendingQuotes.length}</span>}
            </div>
            ...
            <div className="aw-admin-sb-footer">
              ...
              <div><div className="aw-admin-sb-name">Isaac</div><div className="aw-admin-sb-role">Owner · Admin</div></div>
            </div>
          </div>
          ...
          <div className="aw-admin-content">
            <div className="aw-admin-topbar">
              <div className="aw-admin-topbar-title">{topbarTitles[page]}</div>
            </div>

            {/* ── APPLICATIONS ── */}
            {page === 'applications' && (
              <div className="aw-admin-page">
                {loading ? <div className="aw-admin-loading">Loading...</div> : (
                  <>
                    ...
                    <div className="aw-admin-tabs">
                      ...
                      <div className={`aw-admin-tab${appTab === 'all' ? ' active' : ''}`} onClick={() => setAppTab('all')}>All ({apps.length})</div>
                    </div>
                    {filteredApps.length === 0 ? (
                      <div className="aw-admin-empty"><div className="aw-admin-empty-icon">📭</div><div className="aw-admin-empty-text">No applications</div></div>
                    ) : (
                      <div className="aw-admin-table-card">
                        <div className="aw-admin-table-header"><div className="aw-admin-table-title">{appTab === 'pending' ? 'Pending Applications' : 'All Applications'}</div></div>
                        <table className="aw-admin-table">
                          <thead><tr><th>Company</th><th>Contact</th><th>Type</th><th>Volume</th><th>Applied</th><th>Status</th><th>Actions</th></tr></thead>
                          <tbody>
                            {filteredApps.map(app => (
                              <tr key={app.id}>
                                <td><div className="aw-admin-td-bold">{app.company_name}</div><div className="aw-admin-td-sub">{app.state}</div></td>
                                <td><div style={{ fontSize: 12, color: '#94a3b8' }}>{app.first_name} {app.last_name}</div><div className="aw-admin-td-sub">{app.email}</div></td>
              ...
              <div className="aw-admin-page">
                {loading ? <div className="aw-admin-loading">Loading...</div> : (
                  <>
                    ...
                    {msgs.length === 0 ? (
                      <div className="aw-admin-empty"><div className="aw-admin-empty-icon">📭</div><div className="aw-admin-empty-text">No messages yet</div></div>
                    ) : msgs.map(msg => (
            ...
            {page === 'quotes' && (
              <div className="aw-admin-page">
                {quotesLoading ? <div className="aw-admin-loading">Loading...</div> : (
                  <>
                    <div className="aw-admin-stats">
                      <div className="aw-admin-stat"><div className="aw-admin-stat-label">Pending</div><div className="aw-admin-stat-val">{pendingQuotes.length}</div></div>
                      <div className="aw-admin-stat"><div className="aw-admin-stat-label">Total Quotes</div><div className="aw-admin-stat-val">{quotes.length}</div></div>
                      <div className="aw-admin-stat"><div className="aw-admin-stat-label">Total Units</div><div className="aw-admin-stat-val">{quotes.reduce((s, q) => s + q.total_units, 0)}</div></div>
                      <div className="aw-admin-stat"><div className="aw-admin-stat-label">Pipeline Value</div><div className="aw-admin-stat-val" style={{ fontSize: 18 }}>${quotes.filter(q => q.status === 'pending').reduce((s, q) => s + Number(q.total_value), 0).toLocaleString('en-US', { minimumFractionDigits: 0 })}</div></div>
                    </div>
                    {quotes.length === 0 ? (
                      <div className="aw-admin-empty"><div className="aw-admin-empty-icon">📋</div><div className="aw-admin-empty-text">No quote requests yet</div></div>
                    ) : (
                      <div className="aw-admin-table-card">
                        <div className="aw-admin-table-header"><div className="aw-admin-table-title">All Quote Requests</div></div>
                        <table className="aw-admin-table">
                          <thead><tr><th>Ref</th><th>Company</th><th>Contact</th><th>Units</th><th>Value</th><th>Submitted</th><th>Status</th><th>Actions</th></tr></thead>
                          <tbody>
                            {quotes.map(q => (
                              <tr key={q.id}>
                                <td><span style={{ fontFamily: 'monospace', fontSize: 11, color: '#94a3b8' }}>{q.ref_number}</span></td>
                                <td><div className="aw-admin-td-bold">{q.company_name}</div></td>
                                <td><div style={{ fontSize: 12 }}>{q.dealer_name}</div><div className="aw-admin-td-sub">{q.dealer_email}</div></td>
                                <td style={{ fontWeight: 700, color: '#e2e8f0' }}>{q.total_units}</td>
                                <td style={{ fontWeight: 700, color: '#ea580c' }}>${Number(q.total_value).toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
                                <td>{timeAgo(q.created_at)}</td>
                                <td><span className={`aw-admin-badge ${statusBadge(q.status)}`}>{q.status}</span></td>
                                <td>
                                  <div style={{ display: 'flex', gap: 5 }}>
                                    <button className="aw-admin-btn aw-admin-btn-view" onClick={() => { setQuoteDetail(q); setDeclineReason('') }}>View</button>
                                    {q.status === 'pending' && (
                                      <>
                                        <button className="aw-admin-btn aw-admin-btn-approve" onClick={() => updateQuoteStatus(q.id, 'confirmed')}>Confirm</button>
                                        <button className="aw-admin-btn aw-admin-btn-processing" onClick={() => updateQuoteStatus(q.id, 'processing')}>Processing</button>
                                      </>
                                    )}
                                    {q.status !== 'pending' && (
                                      <button className="aw-admin-btn aw-admin-btn-reset" onClick={() => updateQuoteStatus(q.id, 'pending')}>↩ Reset</button>
                                    )}
                                  </div>
                                </td>
                              </tr>
            ...
            {page === 'sync' && (
              <div className="aw-admin-page">
                ...
                <div className="aw-admin-stats">
                  <div className="aw-admin-stat"><div className="aw-admin-stat-label">Connection</div><div className="aw-admin-stat-val" style={{ fontSize: 16 }}>{syncLoading ? '...' : syncStatus?.success ? <span style={{color:'#22c55e'}}>Connected</span> : <span style={{color:'#ef4444'}}>Error</span>}</div></div>
                  <div className="aw-admin-stat"><div className="aw-admin-stat-label">API Endpoint</div><div className="aw-admin-stat-val" style={{ fontSize: 12, color: '#94a3b8' }}>bi.api.sellercloud.com</div></div>
                  <div className="aw-admin-stat"><div className="aw-admin-stat-label">Catalog Products</div><div className="aw-admin-stat-val">{catalogData?.total?.toLocaleString() || '—'}</div></div>
                  <div className="aw-admin-stat"><div className="aw-admin-stat-label">In-Stock SKUs</div><div className="aw-admin-stat-val">{inventoryData?.total?.toLocaleString() || '—'}</div></div>
                </div>
                <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
                  <button className="aw-admin-btn aw-admin-btn-primary" onClick={testSCAuth} disabled={syncLoading}>{syncLoading ? 'Testing...' : '🔄 Test Connection'}</button>
                  <button className="aw-admin-btn aw-admin-btn-view" onClick={() => fetchSCCatalog()} disabled={syncSearching}>{syncSearching ? 'Loading...' : '📦 Load Catalog'}</button>
                  <button className="aw-admin-btn aw-admin-btn-docs" onClick={() => fetchSCInventory()} disabled={syncSearching}>{syncSearching ? 'Loading...' : '📊 Load Inventory'}</button>
                </div>
                ...
                <div style={{ marginBottom: 20, display: 'flex', gap: 8 }}>
                  <input style={{ flex: 1, padding: '10px 14px', background: '#0a0f1a', border: '1px solid #1e2d4a', borderRadius: 8, fontSize: 13, color: '#e2e8f0', fontFamily: "'DM Sans', sans-serif", outline: 'none' }}
                    placeholder="Search products..." value={syncSearch} onChange={e => setSyncSearch(e.target.value)}
                    onKeyDown={e => { if (e.key === 'Enter' && syncSearch.trim()) { fetchSCCatalog(syncSearch); fetchSCInventory(syncSearch) } }} />
                  <button className="aw-admin-btn aw-admin-btn-primary" onClick={() => { fetchSCCatalog(syncSearch); fetchSCInventory(syncSearch) }} disabled={syncSearching || !syncSearch.trim()}>Search</button>
                </div>
                <div className="aw-admin-tabs">
                  <div className={`aw-admin-tab${syncTab === 'overview' ? ' active' : ''}`} onClick={() => setSyncTab('overview')}>Overview</div>
                  <div className={`aw-admin-tab${syncTab === 'catalog' ? ' active' : ''}`} onClick={() => setSyncTab('catalog')}>Catalog {catalogData?.total ? `(${catalogData.total})` : ''}</div>
                  <div className={`aw-admin-tab${syncTab === 'inventory' ? ' active' : ''}`} onClick={() => setSyncTab('inventory')}>Inventory {inventoryData?.total ? `(${inventoryData.total})` : ''}</div>
                </div>
                {syncTab === 'overview' && (
                  <div className="aw-admin-table-card">
                    <div className="aw-admin-table-header"><div className="aw-admin-table-title">Integration Status</div></div>
                    <table className="aw-admin-table">
                      <thead><tr><th>Feature</th><th>Status</th><th>Details</th></tr></thead>
                      <tbody>
                        <tr><td className="aw-admin-td-bold">API Authentication</td><td><span className={`aw-admin-badge ${syncStatus?.success ? 'aw-admin-badge-green' : 'aw-admin-badge-red'}`}>{syncStatus?.success ? 'Active' : 'Not Connected'}</span></td><td>{syncStatus?.success ? `Token expires in ${syncStatus.expiresIn}` : syncStatus?.error || 'Click Test Connection'}</td></tr>
                        <tr><td className="aw-admin-td-bold">Catalog Sync</td><td><span className={`aw-admin-badge ${catalogData?.total > 0 ? 'aw-admin-badge-green' : 'aw-admin-badge-yellow'}`}>{catalogData?.total > 0 ? 'Data Available' : 'Not Loaded'}</span></td><td>{catalogData?.total ? `${catalogData.total.toLocaleString()} products` : 'Click Load Catalog'}</td></tr>
                        <tr><td className="aw-admin-td-bold">Inventory Sync</td><td><span className={`aw-admin-badge ${inventoryData?.total > 0 ? 'aw-admin-badge-green' : 'aw-admin-badge-yellow'}`}>{inventoryData?.total > 0 ? 'Data Available' : 'Not Loaded'}</span></td><td>{inventoryData?.total ? `${inventoryData.total.toLocaleString()} in-stock SKUs` : 'Click Load Inventory'}</td></tr>
                        <tr><td className="aw-admin-td-bold">Website Catalog</td><td><span className="aw-admin-badge aw-admin-badge-yellow">Pending</span></td><td>Not yet wired to live data</td></tr>
                        <tr><td className="aw-admin-td-bold">Auto-Sync Schedule</td><td><span className="aw-admin-badge aw-admin-badge-gray">Not Set</span></td><td>Manual sync only</td></tr>
                      </tbody>
                    </table>
                  </div>
                )}
                {syncTab === 'catalog' && catalogData?.items && (
                  <div className="aw-admin-table-card">
                    <div className="aw-admin-table-header"><div className="aw-admin-table-title">SellerCloud Catalog ({catalogData.total?.toLocaleString()} products)</div></div>
                    <div style={{ overflowX: 'auto' }}>
                      <table className="aw-admin-table">
                        <thead><tr><th>SKU</th><th>Product</th><th>Qty</th><th>Avg Cost</th><th>Sold 30d</th><th>Sold 90d</th><th>Channels</th></tr></thead>
                        <tbody>
                          {catalogData.items.map((item: any, i: number) => (
                            <tr key={i}>
                              ...
                              <td><div className="aw-admin-td-bold">{item.name}</div>{item.parsed && <div className="aw-admin-td-sub">{item.parsed.deviceType} · {item.parsed.manufacturer} · {item.parsed.gradeDescription}</div>}</td>
                              ...
                              <td>{item.sold30 || '—'}</td><td>{item.sold90 || '—'}</td>
                              <td style={{ fontSize: 11 }}>{[item.backMarketEnabled && 'BM', item.ebayEnabled && 'eBay', item.walmartEnabled && 'WM', item.amazonEnabled && 'AMZ'].filter(Boolean).join(', ') || '—'}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                ...
                {syncTab === 'catalog' && !catalogData?.items && <div className="aw-admin-empty"><div className="aw-admin-empty-icon">📦</div><div className="aw-admin-empty-text">Click "Load Catalog" to see products</div></div>}
                {syncTab === 'inventory' && inventoryData?.items && (
                  <div className="aw-admin-table-card">
                    <div className="aw-admin-table-header"><div className="aw-admin-table-title">SellerCloud Inventory ({inventoryData.total?.toLocaleString()} in-stock SKUs)</div></div>
                    <div style={{ overflowX: 'auto' }}>
                      <table className="aw-admin-table">
                        <thead><tr><th>SKU</th><th>Product</th><th>Physical</th><th>Available</th><th>Reserved</th><th>Avg Cost</th><th>Warehouse</th><th>Velocity</th></tr></thead>
                        <tbody>
                          {inventoryData.items.map((item: any, i: number) => (
                            <tr key={i}>
                              ...
                              <td>{item.availableQty}</td><td>{item.reservedQty || '—'}</td>
                              <td>{item.avgCost > 0 ? `$${item.avgCost.toFixed(2)}` : '—'}</td>
                ...
                {syncTab === 'inventory' && !inventoryData?.items && <div className="aw-admin-empty"><div className="aw-admin-empty-icon">📊</div><div className="aw-admin-empty-text">Click "Load Inventory" to see stock</div></div>}
              </div>
            )}
            ...
            {detail && (
              <div className="aw-admin-detail-overlay" onClick={() => setDetail(null)}>
                <div className="aw-admin-detail" onClick={e => e.stopPropagation()}>
                  ...
                  {detail.notes && <div style={{ marginBottom: 24 }}><div className="aw-admin-detail-label">Notes</div><div style={{ fontSize: 13, color: '#94a3b8', lineHeight: 1.6, marginTop: 4 }}>{detail.notes}</div></div>}
            ...
            {msgDetail && (
              <div className="aw-admin-detail-overlay" onClick={() => { setMsgDetail(null); setReplyText('') }}>
                <div className="aw-admin-detail" onClick={e => e.stopPropagation()}>
                  ...
                  <textarea className="aw-admin-reply-textarea" placeholder="Type your reply..." value={replyText} onChange={e => setReplyText(e.target.value)} />
                  <div className="aw-admin-detail-actions" style={{ marginTop: 16 }}>
                    <button className="aw-admin-btn aw-admin-btn-primary" disabled={replySending || !replyText.trim()} onClick={sendReply}>{replySending ? 'Sending...' : '📧 Send Reply'}</button>
            ...
            {/* ── QUOTE DETAIL MODAL ── */}
            {quoteDetail && (
              <div className="aw-admin-detail-overlay" onClick={() => { setQuoteDetail(null); setDeclineReason('') }}>
                <div className="aw-admin-detail" style={{ width: 640 }} onClick={e => e.stopPropagation()}>
                  <div className="aw-admin-detail-title">{quoteDetail.company_name}</div>
                  <div className="aw-admin-detail-sub">
                    {quoteDetail.ref_number} · {timeAgo(quoteDetail.created_at)} · <span className={`aw-admin-badge ${statusBadge(quoteDetail.status)}`}>{quoteDetail.status}</span>
                  </div>
                  <div className="aw-admin-detail-grid">
                    <div className="aw-admin-detail-field"><div className="aw-admin-detail-label">Contact</div><div className="aw-admin-detail-value">{quoteDetail.dealer_name}</div></div>
                    <div className="aw-admin-detail-field"><div className="aw-admin-detail-label">Email</div><div className="aw-admin-detail-value">{quoteDetail.dealer_email}</div></div>
                    <div className="aw-admin-detail-field"><div className="aw-admin-detail-label">Total Units</div><div className="aw-admin-detail-value">{quoteDetail.total_units}</div></div>
                    <div className="aw-admin-detail-field"><div className="aw-admin-detail-label">Est. Value</div><div className="aw-admin-detail-value" style={{ color: '#ea580c' }}>${Number(quoteDetail.total_value).toLocaleString('en-US', { minimumFractionDigits: 2 })}</div></div>
                  </div>
                  <div className="aw-admin-detail-label">Line Items</div>
                  <table className="aw-admin-table" style={{ marginTop: 8, marginBottom: 16, border: '1px solid #1e2d4a', borderRadius: 8, overflow: 'hidden' }}>
                    <thead><tr><th>Product</th><th>Grade</th><th>Spec</th><th>Qty</th><th>Total</th></tr></thead>
                    <tbody>
                      {quoteDetail.items.map((item, i) => (
                        <tr key={i}>
                          <td className="aw-admin-td-bold">{item.name}</td>
                          <td>{item.grade}</td>
                          <td style={{ fontSize: 11 }}>{[item.storage, item.carrier, item.color].filter(Boolean).join(' · ')}</td>
                          <td style={{ fontWeight: 700, color: '#e2e8f0' }}>{item.qty}</td>
                          <td style={{ fontWeight: 700, color: '#ea580c' }}>${(item.price * item.qty).toFixed(2)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  {quoteDetail.notes && (
                    <div style={{ marginBottom: 16 }}>
                      <div className="aw-admin-detail-label">Dealer Notes</div>
                      <div style={{ fontSize: 13, color: '#94a3b8', lineHeight: 1.6, marginTop: 4, padding: 12, background: '#0a0f1a', borderRadius: 8, border: '1px solid #1e2d4a' }}>{quoteDetail.notes}</div>
                    </div>
                  )}
                  <div className="aw-admin-detail-actions">
                    {quoteDetail.status === 'pending' && (
                      <>
                        <button className="aw-admin-btn aw-admin-btn-approve" onClick={() => updateQuoteStatus(quoteDetail.id, 'confirmed')}>✓ Confirm Quote</button>
                        <button className="aw-admin-btn aw-admin-btn-processing" onClick={() => updateQuoteStatus(quoteDetail.id, 'processing')}>Mark Processing</button>
                        <div className="aw-decline-row">
                          <select className="aw-decline-select" value={declineReason} onChange={e => setDeclineReason(e.target.value)}>
                            <option value="">Decline reason...</option>
                            {Object.entries(DECLINE_REASONS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
                          </select>
                          <button className="aw-admin-btn aw-admin-btn-reject"
                            style={{ opacity: declineReason ? 1 : 0.4 }}
                            onClick={() => { if (!declineReason) return; updateQuoteStatus(quoteDetail.id, 'declined', declineReason) }}>
                            ✗ Decline
                          </button>
                        </div>
                      </>
                    ...
                    {quoteDetail.status !== 'pending' && (
                      <button className="aw-admin-btn aw-admin-btn-reset" onClick={() => updateQuoteStatus(quoteDetail.id, 'pending')}>↩ Reset to Pending</button>
                    )}
                    <button className="aw-admin-btn aw-admin-detail-close" onClick={() => { setQuoteDetail(null); setDeclineReason('') }}>Close</button>
                  </div>
                </div>
              </div>
            )}
