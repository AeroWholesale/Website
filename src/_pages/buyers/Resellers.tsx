      ...
      <div className="rs-page">
        ...
        <div className="rs-hero">
          <div className="rs-hero-inner">
            <div>
              <div className="rs-hero-eyebrow">Marketplace Resellers</div>
              <div className="rs-hero-title">Source Smarter. <span>Sell More.</span></div>
              <div className="rs-hero-sub">AeroWholesale supplies marketplace resellers with IMEI-verified, grade-sorted refurbished devices — the consistent, reliable sourcing your listings depend on.</div>
              <div className="rs-hero-btns">
                <button className="rs-btn-primary" onClick={() => navigate('/apply')}>Apply for Reseller Access</button>
                <button className="rs-btn-ghost" onClick={() => navigate('/catalog')}>Browse Catalog</button>
              </div>
            </div>
            <div className="rs-hero-card">
              <div className="rs-hero-card-title">Typical Reseller Margins</div>
              {[
                { grade: 'Premium', margin: '15–25%', cls: 'rs-mp-blue' },
                { grade: 'Excellent', margin: '20–30%', cls: 'rs-mp-green' },
                { grade: 'Good / B-Grade', margin: '25–40%', cls: 'rs-mp-yellow' },
              ].map(r => (
                <div key={r.grade} className="rs-margin-row">
                  <div className="rs-margin-grade">{r.grade}</div>
                  <div className={`rs-margin-pill ${r.cls}`}>{r.margin} margin</div>
                </div>
              ))}
              <div style={{ marginTop: 16, fontSize: 11, color: '#4e6480', lineHeight: 1.5 }}>
                Margins vary by platform, model, and market conditions. Figures are illustrative estimates.
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="rs-trust">
          <div className="rs-trust-inner">
            {['IMEI Clean Guarantee', 'Grade-Sorted Inventory', 'No Activation Lock', 'Marketplace-Ready', 'Fast Fulfillment'].map(t => (
              <div key={t} className="rs-trust-item"><div className="rs-trust-dot" />{t}</div>
            ))}
          </div>
        </div>

        {/* WHY */}
        <div className="rs-section rs-section-alt">
          <div className="rs-section-inner">
            <div className="rs-section-label">Why AeroWholesale</div>
            <div className="rs-section-title">Built for Resellers <br/>Who Move Volume</div>
            <div className="rs-section-sub">Your marketplace reputation depends on what you ship. We make sure every device that leaves our warehouse is ready to list, sell, and satisfy.</div>
            <div className="rs-features">
              {[
                { icon: '✅', title: 'IMEI Clean, Always', desc: 'Every device is verified against blacklists before it ships. No surprises for your buyers, no returns from activation issues.' },
                { icon: '📦', title: 'Grade-Sorted Lots', desc: 'We sort by grade so your listings are accurate. Premium is Premium. Good is Good. No mixed conditions in a single lot.' },
                { icon: '🔋', title: 'Battery Health Disclosed', desc: 'Battery health is tested and graded. Devices with battery under 80% are clearly identified — so you price and list accordingly.' },
                { icon: '⚡', title: 'Fast Replenishment', desc: 'When a listing sells out, you need to restock fast. We carry deep inventory and turn quotes around quickly.' },
                { icon: '💰', title: 'Competitive Pricing', desc: 'Sourcing cost is the biggest lever on your margins. Our wholesale pricing is built to give resellers room to compete.' },
                { icon: '📋', title: 'Consistent Supply', desc: 'We\'re not a one-time liquidator. We maintain ongoing inventory so you can build a repeatable sourcing relationship.' },
              ].map(f => (
                <div key={f.title} className="rs-feature">
                  <div className="rs-feature-icon">{f.icon}</div>
                  <div className="rs-feature-title">{f.title}</div>
                  <div className="rs-feature-desc">{f.desc}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="rs-section">
          <div className="rs-section-inner">
            <div className="rs-section-label">Grading Standards</div>
            <div className="rs-section-title">Know Exactly What <br/>You're Listing</div>
            <div className="rs-section-sub">Our grading is consistent and documented. Each grade maps to clear cosmetic and functional criteria so your listings are accurate every time.</div>
            <div className="rs-grades-table">
              <div className="rs-grades-header">
                <span>Grade</span>
                <span>Condition</span>
                <span>Best For</span>
              </div>
              {[
                { name: 'Premium / Premium 100%', badge: 'rs-gb-blue', badgeLabel: 'Premium', note: 'Like-new cosmetics, battery 80%+ or 100%. Top-tier listings, highest ASP.' },
                { name: 'Excellent', badge: 'rs-gb-green', badgeLabel: 'Excellent', note: 'Light wear, fully functional, battery 80%+. Best volume-to-margin sweet spot.' },
                { name: 'Good', badge: 'rs-gb-yellow', badgeLabel: 'Good', note: 'Moderate wear, fully functional. Budget listings, high-volume movers.' },
                { name: 'B-Grade', badge: 'rs-gb-orange', badgeLabel: 'B-Grade', note: 'Heavier cosmetic wear, functional. Value listings and parts buyers.' },
              ].map(g => (
                <div key={g.name} className="rs-grades-row">
                  <div className="rs-grade-name">{g.name}</div>
                  <div><span className={`rs-grade-badge ${g.badge}`}>{g.badgeLabel}</span></div>
                  <div className="rs-grade-note">{g.note}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="rs-section rs-section-alt">
          <div className="rs-section-inner">
            <div className="rs-section-label">Where You Sell</div>
            <div className="rs-section-title">Inventory Ready for <br/>Any Sales Channel</div>
            <div className="rs-section-sub">Our devices are prepared for resale across all major platforms and formats — cosmetically graded, functionally tested, and marketplace-ready.</div>
            <div className="rs-channels">
              {[
                { icon: '🛒', title: 'Online Marketplaces', desc: 'List with confidence on any resale platform. Grade accuracy and IMEI verification protect your seller metrics and reduce returns.' },
                { icon: '🏪', title: 'Retail & Storefront', desc: 'Physical resellers get consistent stock they can grade-match to their store\'s quality standards.' },
                { icon: '🔄', title: 'Trade-In & Buyback Programs', desc: 'Use our inventory to fulfill trade-in obligations or supplement buyback programs with graded supply.' },
                { icon: '📦', title: 'B2B & Bulk Sales', desc: 'Flip lots directly to other buyers. Our grade-sorted inventory makes it easy to move in bulk without surprises.' },
              ].map(c => (
                <div key={c.title} className="rs-channel">
                  <div className="rs-channel-icon">{c.icon}</div>
                  <div>
                    <div className="rs-channel-title">{c.title}</div>
                    <div className="rs-channel-desc">{c.desc}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="rs-section">
          <div className="rs-section-inner">
            <div className="rs-cta">
              <div className="rs-cta-title">Ready to Source from AeroWholesale?</div>
              <div className="rs-cta-sub">Apply for a reseller account and get access to our full catalog with wholesale pricing. We review applications within 1 business day.</div>
